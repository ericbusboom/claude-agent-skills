...
status: current
sprint: "018"
description: Architecture update for sprint 018 — GitHub issue sync (import, reference tracking, auto-close)
---
...
1. **Process Content Delivery** — Serving SE process definitions (9 agents,
   18 skills, 5 instructions) to AI agents at runtime via MCP tools.
...
- Release: `tag_version`
- GitHub: `create_github_issue`, `list_github_issues`, `close_github_issue`
...
- **18 skills**: plan-sprint, execute-ticket, close-sprint,
  create-tickets, elicit-requirements, project-initiation, gh-import,
  and 11 others
...
| Ticket | `docs/plans/sprints/NNN-slug/tickets/NNN-slug.md` | id, title, status, use-cases, depends-on, github-issue (optional) |
| TODO | `docs/plans/todo/name.md` | status, github-issue (optional) |
...
- GitHub issue creation uses `GITHUB_TOKEN` when available, falling back to
  ...
  in project files.
- All `gh` CLI invocations use `subprocess.run` with list-form arguments
  (never `shell=True`) to prevent command injection via user-supplied
  repo names or issue numbers.
...
1. **`list_github_issues` accepts an optional `repo` parameter** —
   defaults to the current repo (via `_get_github_repo`), but supports
   explicit cross-repo import.

2. **The `## GitHub Issues` section is manually maintained by the agent
   during sprint planning** — the agent collects `github-issue` references
   from tickets when writing the sprint doc. This is more visible and
   consistent with how other sprint doc sections work. The `close-sprint`
   skill reads this section at close time.

## Sprint Changes

Changes planned for sprint 018:

### New Components

**Skill: `gh-import`** — New skill definition (`skills/gh-import.md`)
that instructs the agent to fetch GitHub issues via the `list_github_issues`
MCP tool, apply the >5-issue gate, and create TODO files with
`github-issue` frontmatter. Includes `gh` CLI access verification step.

**MCP Tool: `list_github_issues`** — New tool in `artifact_tools.py`
that wraps `gh issue list` to fetch open issues from a repo. Accepts
optional `repo`, `labels`, `state`, and `limit` parameters. Returns
JSON array of `{number, title, body, labels, url}`. Verifies `gh` CLI
availability and repo access before listing.

**MCP Tool: `close_github_issue`** — New tool in `artifact_tools.py`
that wraps `gh issue close` to close a specific issue. Accepts `repo`
and `issue_number` parameters. Validates `issue_number` is a positive
integer before invoking `gh`. Returns success/failure JSON.

**Helper: `_check_gh_access`** — New private function in
`artifact_tools.py` that runs `gh issue list --repo <repo> --limit 1`
to verify the user's `gh` CLI can access a repo's issues. Returns a
clear error message if access fails, including remediation steps.

### Changed Components

**Reference Propagation (agent-driven)** — The `github-issue` field is
propagated by the agent, not by MCP tools. When the `create-tickets`
skill (technical-lead agent) creates tickets from TODOs that have a
`github-issue` field, the skill instructions tell the agent to copy the
field to the ticket frontmatter. When writing the sprint doc, the agent
collects all `github-issue` references from tickets and lists them in
the `## GitHub Issues` section. This is consistent with how other
frontmatter fields (like `use-cases`) are agent-maintained.

**Skill: `close-sprint`** — Updated skill definition
(`skills/close-sprint.md`) with a new step between verification and
merge: read the sprint doc's `## GitHub Issues` section, parse issue
references, and close each one via `close_github_issue`. Failures are
logged but do not block sprint closure.

**Templates** — The sprint doc template (`templates/sprint.md`) gains
a `## GitHub Issues` section (empty by default). The ticket template
(`templates/ticket.md`) gains an optional `github-issue` frontmatter
field (empty string by default, populated when created from a linked
TODO).

**Artifact Tools** — Tool count increases from 21 to 23 with the
addition of `list_github_issues` and `close_github_issue`.

### Migration Concerns

Non-breaking. Existing TODOs and tickets without `github-issue` fields
are unaffected. The new sprint template section is only relevant for
sprints created after this change. The `close-sprint` skill's GitHub
issue step is a no-op when no issues are referenced.
