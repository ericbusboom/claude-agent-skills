...
status: current
sprint: "023"
description: Architecture update for sprint 023 — parallel execution via git worktrees
---
...
Changes planned for sprint 023:

### New Components

**Skill: `parallel-execution`** — New skill defining the parallel ticket
execution workflow. Analyzes tickets for independence (no overlapping
files, no dependency edges), creates worktrees, dispatches subagents
(using dispatch-subagent from sprint 022), collects results, reviews,
merges back. Opt-in only — sequential remains default.
Reference: Superpowers `dispatching-parallel-agents.md`.

**Instruction: `worktree-protocol`** — New instruction defining worktree
management rules. Naming convention (`worktree-ticket-NNN`), cleanup
after merge, conflict resolution procedure (fall back to sequential).
Reference: Superpowers `using-git-worktrees.md`.

### Changed Components

**Skill: `execute-ticket`** — Updated with note about parallel execution
option and when it's appropriate.

**Agent: `project-manager`** — Updated with parallel dispatch guidance
for sprints with independent tickets.

### Migration Concerns

Non-breaking. Content-only changes. The execution lock remains at sprint
level — no database changes.
