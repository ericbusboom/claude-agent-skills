...
status: current
sprint: "022"
description: Architecture update for sprint 022 — subagent dispatch protocol and two-stage review
---
<!-- CLASI: Before changing code or making plans, review the SE process in CLAUDE.md -->

# Architecture 022: Subagent Dispatch and Two-Stage Review
...
Changes planned for sprint 022:

### New Components

**Skill: `dispatch-subagent`** — New skill defining the controller/worker
pattern for subagent dispatch. Controller curates context, composes prompt,
dispatches via Agent tool, reviews results, iterates if needed.
Reference: Superpowers `subagent-driven-development.md`.

**Instruction: `subagent-protocol`** — New instruction defining context
curation rules. Include: relevant source files, ticket, acceptance
criteria, architecture decisions, coding standards. Exclude: conversation
history, other tickets, debugging logs, full file listings.

**Template: `review-checklist`** — New template for structured code
review output. Two sections: correctness (pass/fail per criterion) and
quality (severity-ranked issues). Stored as `NNN-slug-review.md`
alongside tickets.

### Changed Components

**Agent: `code-reviewer`** — Restructured into two explicit phases.
Phase 1: correctness vs acceptance criteria (binary pass/fail). Phase 2:
quality vs coding standards (severity-ranked). Phase 1 failure
short-circuits Phase 2.

**Agent: `project-manager`** — Updated to use dispatch-subagent as
primary execution method for ticket implementation.

**Skill: `execute-ticket`** — Modified to dispatch a coding subagent
via Agent tool rather than switching personas inline.

**Templates module (`templates.py`)** — New constant for review
checklist template. Minor Python code change.

### Migration Concerns

Non-breaking. Changes the execution model for future sprints but
doesn't affect existing artifacts or state.
