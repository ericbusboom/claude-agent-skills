...
1. **Should `list_github_issues` accept a `repo` parameter, or always
   use the current repo?** — The stakeholder's use case involves issues
   being filed on *this* repo by external LLMs, suggesting current-repo
   is the default. But supporting an explicit `repo` parameter adds
   flexibility for cross-repo import at minimal cost.

2. **Should the `## GitHub Issues` section in the sprint doc be
   auto-generated by `close_sprint`, or manually maintained by the
   agent during sprint planning?** — Auto-generating at close time
   means scanning all tickets for `github-issue` fields. Manual
   maintenance during planning is more visible but risks drift.
...
and `issue_number` parameters. Returns success/failure JSON.
