...
status: current
sprint: "021"
description: Architecture update for sprint 021 — session-start hook for mandatory SE process loading
---
<!-- CLASI: Before changing code or making plans, review the SE process in CLAUDE.md -->

# Architecture 021: Session-Start Hook
...
1. **What is the exact Claude Code hooks mechanism?** — Need to verify
   whether hooks use `.claude/settings.local.json`, a dedicated hooks
   directory, or another format. This must be confirmed before
   implementation. Reference Superpowers' approach and Claude Code docs.

## Sprint Changes

Changes planned for sprint 021:

### New Components

**Hook configuration** — A session-start hook that outputs a message
triggering the agent to call `get_se_overview()`. Exact format depends
on the Claude Code hooks mechanism (to be confirmed during implementation).

### Changed Components

**Init Command (`init_command.py`)** — Modified to create the hook
configuration during `clasi init`. Must be idempotent (re-running init
doesn't duplicate the hook). This is a Python code change.

### Migration Concerns

Re-running `clasi init` on existing projects should install the hook
without affecting other init outputs. Projects initialized before this
sprint will get the hook on next `clasi init` run.
