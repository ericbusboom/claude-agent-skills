...
status: current
sprint: "019"
description: Architecture update for sprint 019 — TDD cycle and systematic debugging skills
---
<!-- CLASI: Before changing code or making plans, review the SE process in CLAUDE.md -->

# Architecture 019: TDD and Systematic Debugging
...
- **19 skills**: plan-sprint, execute-ticket, close-sprint,
  create-tickets, elicit-requirements, project-initiation, tdd-cycle,
  systematic-debugging, and 11 others
...
Changes planned for sprint 019:

### New Components

**Skill: `tdd-cycle`** — New skill definition (`skills/tdd-cycle.md`)
defining the red-green-refactor TDD workflow. Available as an opt-in
method during ticket implementation. Covers: writing failing tests,
confirming failure, writing minimal code, confirming pass, refactoring.
Includes guidance for unexpected test passes and edge cases.
Reference: Superpowers `tdd.md`.

**Skill: `systematic-debugging`** — New skill definition
(`skills/systematic-debugging.md`) defining a four-phase debugging
protocol: evidence gathering, pattern analysis, hypothesis testing,
root cause fix. Three-attempt cap before escalation. Includes audit
trail requirements (document evidence and hypotheses).
Reference: Superpowers `systematic-debugging.md`.

### Changed Components

**Skill: `execute-ticket`** — Updated to reference tdd-cycle as an
available option during implementation, and systematic-debugging when
tests fail or implementation hits problems.

**Instruction: `testing`** — Updated to reference TDD as an available
development method alongside the existing implement-then-test approach.
Includes guidance on when TDD is most useful.

**Instruction: `software-engineering`** — Updated to reference the
debugging protocol.

### Migration Concerns

Non-breaking. Content-only changes to bundled skills and instructions.
No Python code, templates, or database changes.
